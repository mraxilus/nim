## Algebra's own vocabulary: what object is, and named incidence questions visualiser asks.
##
...
##
## **Multivector in, multivector or scalar out.** Nothing here names Euclidean quantity;
## lifting one in or reading one out is `boundary.nim`'s job alone. See `euclid.nim` header
## for which side owns what.
##
...
func isHorizonPlane*(m: Multivector): bool = shape(m) == some(Shape.Plane) and isHorizon(m)
  ## Report whether object is plane at horizon: one shape drawn as sky dome
  ## (`mesh.addDome`), which frame assembly inserts before anything else sharing
  ## translucent wash pass. See `visualiser.assembleMeshes` on draw order.



#[ Incidence Vocabulary ]#

# Named incidence questions visualiser asks, each stated once through library's own
#   operators; each returns multivector for further algebra or scalar read out at end.

func planeThrough*(point, direction: Multivector): Multivector =
  ## Build plane through `point` perpendicular to line through it along `direction`,
  ## unitized so `depthAgainst` reads metric distance off it.
  ##   Weight expansion of point onto own line: `p ∧ (p ∧ d)☆` is plane containing `p`
  ## whose normal is line's direction -- library's `expandWeight`, exposed by apply
  ## catalogue as `𝐦 ∧ 𝐧☆`.
  unitize(expandWeight(point, wedge(point, direction)))


func depthAgainst*(plane, point: Multivector): float =
  ## Measure signed distance of unit-weight point from unitized plane: positive on side
  ## plane's construction direction points toward, zero on it.
  ...
  ##   **This argument order**: `plane ∨ point`; other order negates. Suite pins point one
  ## unit along plane's direction at exactly +1.
...
func centroidFolded*(centroid: Multivector; place: Multivector): Multivector =
  ...
  ##   **Sum of unit-weight points is their centroid.** Sum of `n` unit points carries
  ## weight `n` and total of coordinates; `boundary.position` divides by signed weight and
  ## yields mean. No averaging is written anywhere.
  ##   Running sum stays multivector rather than being read to place and lifted per fold,
  ## so accumulated weight *is* count. Caller starts from first point and folds rest in,
  ## allocating nothing -- shape `camera.widened` takes too.
  add(centroid, place)


func distanceBetween*(p, q: Multivector): float =
  ## Measure distance between two unit-weight points.
  ##   Weight norm of joining line: for unitized points join's direction lives in weight,
  ## whose length is separation itself -- `‖p ∧ q‖∘`, read from norm's `scalarAnti` slot.
  normWeight(wedge(p, q))[Basis.scalarAnti]


func levelPlaneThrough*(point: Multivector): Multivector =
  ## Build horizontal plane through `point` -- level reader works at when camera targets
  ## it -- oriented so `depthAgainst` reads **up as positive**.
  ##   Joined y-then-x: probed, x∧y join reads point one unit above at -1, y∧x at +1, and
  ## height is what every caller means. Unitized for `depthAgainst`'s contract.

...
##
## Free slots thread onto intrusive singly-linked list, `next_free`, so `addItem` and
## `removeItem` run in constant time however many items scene holds.
##   Link lives inside slot array itself; dead slot's `next_free` costs nothing beyond
##   what slot carries while alive.
##   Removal shifting every later item down kept indices dense, but every cross-frame index
##   GUI holds (operands picked, item hovered, item mid-drag) went stale. Stable slot
##   narrows that to removed item's own references, without generation counter.
##
## Operation catalogue is what makes scene live rather than scripted: every entry is one
## of library's named aliases, applied to items user picks.
##
##   |---------|--------------------------------|--------------------------------------|
...
# Allow caller to resize scene without editing source, e.g.
#   `--define:visualiser.items_max=128 --define:visualiser.label_max=48`.
const
  ITEMS_MAX* {.define: "visualiser.items_max".} = 5040
    ## Max items scene may hold at once.
    ##   Raised from 64 so demo holds real solar neighbourhood; see `orrery`. Four
    ## capacities in `mesh` are sized against this and checked below, since `mesh` cannot
    ## see it from where it sits in import order.
  LABEL_MAX* {.define: "visualiser.label_max".} = 40
    ## Max characters label may hold.
...
static:
  ...
  # Tie mesh's capacities to this one where both are visible: overflowing any is
  #   `doAssert` at draw time, dead page, so raising `ITEMS_MAX` fails to *compile*.
  ...
  # Bind on scene of *lines*: rim is one ring record, so what fills ribbons is two
  #   segments `tessellate.addLine` steps out per anchor, drawn twice.
...
type
  ...
  Item* = object ## Handle onto one live slot's data; view on native builds, copy under JS
    ## backend, where value parameter's address does not carry across calls.
    ##   Holds pointer into `scene`'s storage plus slot number; reading `.geometry`,
    ## `.label`, `.ink`, `.isVisible` or `.born` resolves into that storage each time. Do
    ## not hold one across mutation of its own slot (`removeItem` then `addItem`).
    when defined(js):
      scene: Scene
    else:
      scene: ptr Scene
  ...
  Scene* = object ## Hold fixed-capacity arena of items, addressed by stable slot.
    ...
    borns: array[ITEMS_MAX, float] ## Per-slot moment item was added, for appear
      ## animation. Not written to scene file -- clock reading means nothing across runs
      ## -- but *loaded* item is stamped all same, so file replays own construction; see
      ## `bornReplaying`.
    revisions_placing: array[ITEMS_MAX, int] ## Per-slot revision at which slot's
      ## placing inputs -- geometry, anchor override -- last changed; see `revisionPlacingAt`.
    orders: array[ITEMS_MAX, uint32] ## Per-slot creation ordinal: how many items scene
      ## had ever been given when this one arrived.
      ##   Separate from `borns` because clock reading cannot answer this: two items added
      ## in one frame share reading, loaded readings are stamped for replay, and refilled
      ## slot keeps old reading until overwritten. Ordinal only ever increases, is never
      ## reused, and survives save and load because file's item sequence *is* this order.
    ...
    anchor_overrides: array[ITEMS_MAX, Option[Position]] ## Where plane's circle should
      ...
      ## closest-to-origin support; see `creationAnchor`. None for anything else. Not saved
      ## or loaded: rendering hint recomputed from how item was built.
    ...
    index_ink: int ## How far categorical cycle has been walked; next hue to hand out.
      ...
      ## palette** -- reader watched colour on band and it should not be offered again.
      ## Undo restores it with rest of scene. Not written to file: `loadScene` sets it from
      ## item count.
...
const lut_operation_to_notation* = [
  ...
  ##   Operands in Lengyel's mathematical bold, every symbol's placement his, written with
  ## spacing modifier letters (`ˆ` U+02C6, `ˍ` U+02CD, `¯` U+00AF, `˜` U+02DC, `˷` U+02F7)
  ## rather than combining marks: combining marks need shaper Dear ImGui lacks, landing
  ## beside operand instead of over it. Spacing modifier carries own advance, so both
  ## renderers place it same way. Not second plain-ASCII table: atlas merges faces
  ## carrying astral-plane glyphs (see `visualiser.PATH_FONT_MATH`).
...
let lut_operation_to_notation_c* = block:
  ## Same entries as `cstring`, what picker offers: Dear ImGui takes address of first and
  ## reads them for life of combo. Built from table above rather than written beside it.
...
const lut_operation_split* = block:
  ## Each operation's two halves: symbols it is written with, and English name after them.
  ## **One split, at one place** -- double space between them -- so no caller cuts at
...
func notationSymbolic*(operation: Operation): string =
  ## Report symbols operation is written with, without English name -- `𝐦 ∧ 𝐧`, not
  ## `𝐦 ∧ 𝐧  wedge (join)`.
  ##   What picker offers on both front-ends: full entry is three to five times wider, and
  ## pushed selection menu's popover past what hand can reach on phone.
  lut_operation_split[operation].symbols


func notationNamed*(operation: Operation): string =
  ## Report English name operation is offered under -- `wedge (join)`, not `𝐦 ∧ 𝐧`. Other
  ## half of `notationSymbolic`'s split.
...
proc notationSubstituted*(operation: Operation; name_first, name_second: string): string =
  ## Build label text applied operation reads as, substituting template's `𝐦`/`𝐧` with
  ## real operand names -- `Operation.Wedge` with "a"/"b" gives "a ∧ b".
  ##   Matches bold operands, not plain ASCII `m`/`n`: English description after symbols
  ## contains ordinary `m` and `n` constantly.
  ##   Swaps through two passes via sentinel bytes no label contains, so operand name
  ## containing placeholder is never re-touched, and template where `𝐧` appears twice
  ## (ProjectCentral/ProjectOrthogonal) substitutes both.
  const
    OPERAND_FIRST = "𝐦"
    OPERAND_SECOND = "𝐧"
    SENTINEL_M = "\x01"
...
func creationAnchor*(operation: Operation; m, n, derived: Multivector): Option[Position] =
  ## Resolve point freshly derived plane's circle should centre on, from how it was
  ## built, rather than from closest-to-origin support -- so it reads as centred where
  ## construction happened. Computed through same operators construction used.
  ##   None for operation or operand shape not recognised here; caller falls back to
  ## plane's support (`objects.positionAnchor`).
  case operation
  of Operation.Wedge:
    # Centre between point and line's closest approach to it: line wedged with point
    #   gives plane line lies within, meeting at no single point. Both unitized first, so
    #   sum's weight is exactly two and position read back is plain midpoint.
    let (line, point) =
...
const lut_basis_to_name* = block:
  ## Name each basis element as library's `$` names it: `𝟏` for scalar, `𝟙` for
  ## antiscalar, bold `𝐞` carrying subscript digits for rest.
  ...
  ## Rule is second copy of one inside `pga/multivectors.nim`'s `$`, which does not expose
  ## it; check that one whenever this is touched.
  const
...
const
  WIDTH_TERM = 32
    ## Bound one printed term in *bytes*: separator, magnitude at `DIGITS_SIGNIFICANT`
    ## with sign and exponent, space, and basis name in mathematical bold with subscripts
    ## at up to 13 bytes. Bytes, since that is what buffer holds.
...
proc formatMultivector*(m: Multivector; storage: var openArray[char]; cursor: var int) =
  ...
  ## those codepoints. Magnitudes stay project's four significant digits.
...
func multivectorText*(m: Multivector): string =
  ...
  ##   Wraps `formatMultivector`: browser's item list once showed library's `$` at
  ## library's `%G`, so same object read differently in two front-ends.
  var
...
func shapeText*(m: Multivector): string =
  ...
  ## browser item list cannot drift. Allocating is affordable here: called on user
  ## action, not per visible item per frame.
  var
...
const ELLIPSIS_LABEL = "…"
  ...
  ##   Three bytes of buffer it warns about: derived names compound, so few steps in every
  ## name is long enough to cut, and one ending mid-word reads as name someone chose.
...
when not defined(js):
  template toCstring*(storage: untyped): cstring = cast[cstring](unsafeAddr storage[0])
    ...
    ##   Desktop-only, guarded rather than policed by comment: address is meaningless on
    ## JS backend, so shared module reading storage as text wants `toText`.
...
func bound*(scene: Scene): int = scene.slot_live_last
  ...
  ##   **What walk over "every slot" runs to.** Slots are stable addresses, so by-slot
  ## reader sweeps range rather than dense list, and at capacity 5,040 that meant testing
  ## five thousand slots per frame to draw five. High-water mark rather than live count,
  ## because freed slot in middle leaves ones above occupied; only ever rises.
...
func revision*(scene: Scene): int = scene.count_edits
  ...
  ##   **For holding last frame's meshes and placements, nothing else.** Front-end
  ## compares against what it saw last frame -- equality only -- and rebuilds where it
  ## differs. Not version number user sees, not saved.
  ...
  ## `replayFrom` -- all here, since fields are private. Label is not among them: labels
  ## are never tessellated.
  ##   **Never assign whole scene over live one; go through `restoreFrom`.** Assignment
  ## restores snapshot's own revision, and bump after it lands on number already seen --
  ## edit being undone. Reader holding meshes on that number then drew undone object
  ## until camera moved; measured on built page before `restoreFrom` existed.

proc markEdited*(scene: var Scene) =
  ## Say that scene's drawn content just changed; see `revision`.
  ##   Called by every writer in this module. Caller wanting this for anything else is
  ## writing to scene by route that ought to be proc here.
...
proc restoreFrom*(scene: var Scene; snapshot: Scene) =
  ...
  ##   Every whole-scene replacement -- undo, redo, clear, load -- comes through here, so
  ## revision only ever rises and no two states front-end has drawn share one. Every live
  ## slot is stamped as re-placed, since any of them may differ from what cache holds.
...
func revisionPlacingAt*(scene: Scene; slot: int): int =
  ...
  ## Re-placing whole scene per edit cost 42 ms frame at 5,038 objects.
...
func isAlive*(scene: Scene; slot: int): bool =
  ...
  ##   Two comparisons, not `slot in 0 ..< ITEMS_MAX`: JS backend built slice object per
  ## call, and every by-slot reader asserts through here, so one moving frame at 5,038
  ## objects allocated twenty thousand of them.
...
func anchorOverride*(item: Item): Option[Position] = item.scene.anchor_overrides[item.slot]
  ## Read where item's circle should centre, if construction fixed that; see
  ## `creationAnchor`.


func geometryOf*(scene: Scene; slot: int): lent Multivector =
  ## Read item's geometry in place, by slot, without mutable scene.
  ##   `lent`, not `var`: `var`-returning accessor read rather than written miscompiled
  ## under JS backend once; borrow cannot be written through.
  ##   **`lent` pays only where result is never bound.** Returning by value allocated fresh
  ## `Multivector` and `nimCopy`d field into it on JS backend, once per call -- fifth of
  ## hover pick's profile. Confirmed in generated JavaScript that `lent` removes copy
  ## **and binding result to `let` puts it back**, so caller wanting saving uses call
  ## inline.
...
func isVisible*(scene: Scene; slot: int): bool =
  ...
  ## confirmed that proc handing back `var bool` over `array[N, bool]` miscompiles under
  ## JS backend, reading `undefined` and writing to dropped copy. `setVisible` is writer.
...
func inkAt*(scene: Scene; slot: int): Ink =
  ...
  ##   Beside `Item.ink` for caller reading many items per frame: under JS backend `Item`
  ## holds `Scene` by value, so constructing one to read single field copies whole scene.
  doAssert scene.isAlive(slot), &"Item slot must be alive; got `{slot}`."
  scene.inks[slot]
...
func siftDown(scene: Scene; slots: var array[ITEMS_MAX, int]; root, count: int) =
  ## Sink slot at `root` until neither child carries later ordinal, over first `count`.
  var parent = root
  while true:
...
func slotsCreated*(scene: Scene; slots: var array[ITEMS_MAX, int]): int =
  ...
  ##   Heapsort on `orders`, in place. Runs per drawer refresh on browser and per edit on
  ## desktop, not once per save as first assumed: insertion sort here was 12.7 million
  ## comparisons per call at 5,038 items, 98% of desktop frame while `panel` ran its own
  ## copy per frame.
...
type Preview* = object ## Hold what applying operation would build, ready to draw and frame.
  ...
  anchor*: Option[Position] ## Where plane's disc should centre, from `creationAnchor`;
    ## none for every other shape. Carried so ghosted plane is drawn exactly where commit
    ## will put it.
  operands*: Option[(int, int)] ## Slots this was derived from, for camera framing
    ## preview to keep in view beside it.
...
func previewApplying*(
  ...
  ##   None where either slot is dead -- picker left open across delete is ordinary --
  ## and none where result has no drawable shape, covering wrong grades and pair already
  ## lying on each other.
...
proc setVisible*(scene: var Scene; slot: int; is_visible: bool) =
  ...
  ##   Only writer: `isVisibleAt(...) = visible` accessor silently landed on copied
...
iterator items*(scene: Scene): Item =
  ...
  ##   Walks to `bound`, as sibling `pairs` does. It was left at capacity when `pairs` was
  ## converted, why sibling rule is written down.
  for slot in 0 ..< scene.bound:
...
iterator pairs*(scene: Scene): (int, Item) =
  ...
  ##   Walks to `bound` rather than capacity, one edit making every desktop consumer
  ## cheap at once: at capacity 1,024 scene of five tested thousand slots per pass.
  for slot in 0 ..< scene.bound:
...
proc addItem*(
  ...
  ##   `now` is stamped as item's `born` reading; default reads as "born at dawn of time"
  ## and never animates.
  ...
  # Stamp arrival relative to everything else, which slot cannot say: refilled slot sits
  #   wherever free list put it.
...
## Binary format project invents for itself. **Every multi-byte field is little-endian**,
## rule rather than habit: `glue.js` writes and reads same file through `DataView`, which
## demands explicit order and is given `true`. Host-native layout left two agreeing only
## because every machine so far is little-endian; on big-endian one desktop would write
## file its own browser build could not read.
...
##   `born` is not written: clock reading meaningless across runs. Loaded item is stamped
## by `bornReplaying`, so file plays back own construction. Label is written as exactly as
## many bytes as it holds, not padded to `LABEL_MAX`.
##
## **Every version ever written is still readable.** Scene file is reader's own work;
## build refusing it has destroyed it. What each older version costs to read:
##
##   |---------|-------------------------------------------------------------------|
##   | Version | Read as                                                           |
##   |---------|-------------------------------------------------------------------|
...
##   |---------|-------------------------------------------------------------------|
##
## **Old file is upgraded to today's shape, never read in old build's dialect.** Reading is
## written once, against `VERSION_SCENE`; everything past version did differently lives in
## one `upgradedFrom<n>` per boundary, and `itemUpgraded` walks items up chain one step at
## time. Reader branching on version at each field spreads every past decision across
## whole reader, and version that breaks is one nobody has file of to notice. Adding
## version means adding one func.

const
  MAGIC_SCENE* = "RGAS"
    ## First four bytes of `.rgascene` file.
  VERSION_SCENE* = 3'u8
    ## Format version this build writes. Every version down to `VERSION_SCENE_LEAST` is
    ## still read; see table above.
    ##   Bumped from 1 when `Ink` gained reserved `Invalid` slot and lost three categorical
    ## ones, moving every stored ordinal. Bumped from 2 when items began to be written in
    ## creation order: bytes are shaped identically, so this number alone says whether
    ## sequence is build order or slot order.
...
func inkCycled*(index: int): Ink = inkCategorical(index mod COUNT_INK_CATEGORICAL)
  ## Choose palette slot for item at given position, cycling categorical slots -- same
  ## run colour picker offers, so nothing cycles to colour user could not have chosen.
...
func inkNext*(scene: Scene): Ink = inkCycled(scene.index_ink)
  ...
  ## built will be; see `interaction.inkOfDrag`. Peeking and taking are separate because
  ## previewing happens every frame.
...
const
  ORDINAL_INK_CATEGORICAL_V1* = 7
    ## Where categorical hues began in `Ink` version-1 file was written under.
    ##   That palette ran `Backdrop, AxisX, AxisY, AxisZ, Grid, Guide, Outline` then eight
    ## hues `Rose, Copper, Olive, Jade, Cobalt, Violet, Magenta, Cerise`. Seven structural
    ## slots are unchanged; every structural slot reserved after them -- `Invalid`, then
    ## `Algebra` -- moves hues along, so fold reads today's start through `inkCycled`.
    ## Recorded as number because enum it indexes no longer exists.
  ORDINAL_INK_HIGH_V1* = 14
    ## Last palette slot version-1 file could name, `Cerise`'s.
...
type ItemSaved* = object
  ## Hold one item exactly as scene file holds it, at whatever version wrote file.
  ##   Shape reading works in, and thing `upgradedFrom<n>` carries between versions.
  ## Distinct from `Item`: value read off bytes that may not describe anything this build
  ## can make yet.
...
func upgradedFrom1(item: ItemSaved): Option[ItemSaved] =
  ...
  ##   Only palette moved: version 1's hues began earlier and ran three longer. Seven
  ## structural slots untouched, five surviving hues land on today's first five, three
  ## retired ones fold onto hues by same cycle `inkCycled` walks.
  ##   **One file this gets wrong** is browser's own, which stamped version 1 onto
  ## version-2 content for while (see `MAGIC_SCENE`); nothing in bytes tells it apart.
  ## Its colours come back one hue along; saving restamps it. Treating version 1 as 2
  ## would *refuse* genuine version-1 file whose `Magenta` and `Cerise` fall past palette.
...
func upgradedFrom2(item: ItemSaved): Option[ItemSaved] = some(item)
  ...
  ##   Version 3 changed only what *sequence* promises, and item alone carries no
  ## sequence. Version-2 file's order is taken as creation order, closest thing it has.
  ##   Kept as explicit step so chain has one entry per boundary.


func itemUpgraded*(item: ItemSaved; version: uint8): Option[ItemSaved] =
  ## Carry item read from file of `version` up to shape this build works in, one boundary
  ## at time. None where no version could have written it.
  ##   On success every field is at `VERSION_SCENE`'s meaning, so caller may take
  ## `Ink(ink_ordinal)` without further check -- last guard here buys that, checked once
  ## at end.
...
const
  SECONDS_REPLAY_STEP* = 0.12
    ## Beat between one loaded object appearing and next.
  ...
  SECONDS_REPLAY_WHOLE* = 2.5
    ## Longest whole replay may take, however many objects arrive.
    ##   Full `ITEMS_MAX` scene would otherwise take nearly eight seconds; beat shortens
    ## instead, keeping order legible while bounding wait.


func bornReplaying*(index, count: int; now: float): float =
  ## Stamp `index`-th of `count` objects arriving together, so they appear one after
  ## another from `now`.
  ##   One rule both loaders use -- desktop's `loadScene` and browser's
  ## `browser_bridge.nimSceneAddRaw` -- because beat computed twice drifts.
  ##   `count` is whole arrival, so beat can be shortened to fit cap; caller not knowing
  ## whole passes own index plus one and gets unbounded beat.
  let step =
...
proc replayFrom*(scene: var Scene; now: float) =
  ## Restamp every live item to arrive one after another from `now`, oldest creation
  ## first, so scene assembled at once plays back as construction it is.
  ##   For arrivals reader did not build and is about to be shown whole -- opening scene
  ## and demo preset. Scene built by hand never wants this.
...
## Constants above sit **outside** desktop-only guard below, exported, because they
## describe *format* rather than file handling: browser build gets them via
## `browser_bridge.nimSceneMagic`/`nimSceneVersion` rather than literals of own.
##   Version was once hand-copied into `glue.js` as `1`; bump to 2 here left browser
## stamping version 1 on version-2 content and refusing every desktop file. Derived value
## behind export cannot drift; literal in other language did.

when not defined(js):
  # Import filesystem here, under guard excluding save and load, so JS build never warns
  #   about imports nothing on its path uses. `endians` joins them because byte order is
  #   property of file.
  ...
  proc readLittle[T](file: File; value: var T): bool =
    ## Read one multi-byte little-endian field into host order; report whether file held
    ## that many bytes, so caller can name truncation.
    var bytes: array[sizeof(T), byte]
  ...
  proc loadScene*(scene: var Scene; path: string; now: float = 0.0): string =
    ...
    ##   `now` is clock arrival is staggered from (see `bornReplaying`); default lands
    ## every object long in past, fully grown.
    ...
    for index in 0 ..< int(count):
      ...
      # Read at file's version, then carry up to this build's; every field below means
      #   what `VERSION_SCENE` says.
    ...
    # Carry palette on past what was loaded, so next object built does not repeat first
    #   object's hue. Not stored in file: count places cycle.
    staging.index_ink = int(count)
    scene = staging
    &"Loaded {count} object(s) from `{path}`."


type OperationMemory* = object ## Remember operation last applied, one per arity.
  ## So picker opens on what reader last reached for: reader applying five wedges in row
  ## picks operation once.
...
const
  OPERATION_FIRST_UNARY* = Operation.Attitude
    ...
    ##   One unary operation whose result is drawn somewhere new rather than on top of
    ## own operand.
...
func remember*(memory: var OperationMemory; operation: Operation) =
  ...
  ##   Call from every path applying one, drag menu's `more…` handover included, or
  ## picker forgets what was reached for by another route.
  if not memory.is_started:

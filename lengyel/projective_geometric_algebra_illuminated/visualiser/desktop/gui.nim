...
##
## Dear ImGui is depended on rather than derived: immediate-mode widget set is external
## concern, like windowing and drivers. Its interface is C++ with overloads and default
## arguments, which Nim cannot import, so `gui_shim.cpp` flattens slice used into C entry
## points and this module binds them. Cost is that adding widget touches two files.
##
## Dear ImGui and its SDL3 and OpenGL 3 backends are compiled straight into binary, so no
## prebuilt library has to be found at link time. Sources are expected at `PATH_IMGUI`;
...
# Allow caller to point at Dear ImGui checkout elsewhere, e.g.
#   `--define:visualiser.path_imgui=/usr/src/imgui`.
...
# Widen `ImWchar` to 32 bits, which Dear ImGui offers and defaults off. Notation uses
#   Lengyel's bold operands (`𝐦`, U+1D426), and 16-bit `ImWchar` cannot express codepoint
#   past U+FFFF. Compiler flag rather than edit to checkout's `imconfig.h`: checkout is
#   build-time dependency never committed, so edit would not survive reclone.
...
proc isNavEnabled*(): bool {.importc: "guiIsNavEnabled".}
  ...
  ## control reachable by Tab.
proc wantsKeys*(): bool {.importc: "guiWantsKeys".}
  ## Report whether key belongs to Dear ImGui rather than 3D view; see shim on why this
  ## is not `wantsKeyboard`.
...
proc windowBeginPinned*(name: cstring; x, y, pivot_x, pivot_y: cfloat): bool
  ...
  ## Begin undecorated window pinned to `x`/`y`, `pivot` naming which of its corners that
  ## is; closed with `windowEnd`.
...
proc childHeightForRows*(count: cint): cfloat {.importc: "guiChildHeightForRows".}
  ## Report how tall bordered `childBegin` region holding `count` lines of plain text must
  ## be, measured against font loaded.
...
proc tabBegin*(label: cstring; is_forced: bool): bool {.importc: "guiTabBegin".}
  ## Begin one tab in row; true only for tab open, and closed with `tabEnd` **only
  ## then**. `is_forced` opens it regardless of what reader last chose; see shim.
...
proc sameLineAt*(offset: cfloat) {.importc: "guiSameLineAt".}
  ## Continue current line at fixed distance from its start, so column of controls lines
  ## up whatever length of each name.
proc groupBegin*() {.importc: "guiGroupBegin".}
  ## Start treating what follows as one item, so name stacked over its control advances
  ## `sameLine` by width of pair.
proc groupEnd*() {.importc: "guiGroupEnd".}
proc buttonSmallWidth*(label: cstring): cfloat {.importc: "guiButtonSmallWidth".}
  ## Report width `buttonSmall` would draw this label at, for caller that must know before
  ## placing anything.
...
proc alignRight*(width: cfloat) {.importc: "guiAlignRight".}
  ## Continue current line with `width` reserved against right edge, so run of controls
  ## ends flush there.
...
proc poolBar*(colours: ptr cfloat; count: cint; cell_size: cfloat)
  {.importc: "guiPoolBar".}
  ## Draw one square cell per pool slot, wrapped to panel's width. `colours` addresses
  ## `count` * 3 floats, red then green then blue per cell.
...
proc overlayCircle*(
  ...
  ## Stroke whole circle. `is_over_windows` picks layer: zero for mark on object, which
  ## panels cover; one for drag menu's centre dot, part of control being steered. See
  ## shim's `overlayList`.
proc overlayArc*(
...
proc overlayPolyline*(
  ...
  ## pairs; one call rather than segment each, so corners join cleanly.
proc overlayRibbon*(
  points: ptr cfloat; count: cint; red, green, blue, alpha: cfloat
) {.importc: "guiOverlayRibbon".}
  ## Fill one closed outline through `count` points, for mark whose width varies along
  ## its length: orientation pulse, and drag band's head.

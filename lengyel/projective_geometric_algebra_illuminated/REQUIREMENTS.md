...
right pans, wheel zooms **toward what the pointer is over** — the map reading of a zoom, so a
reader aims once rather than zooming, panning and zooming again. The point aimed at is where
the sight ray meets the horizontal plane through the target; where there is none — the sky,
or a ray running along that level — the zoom falls back to the middle of the frame.
**A pinch is the exception and stays centred**: the same gesture already pans the view by its
own midpoint, so aiming the zoom there too moves the view twice at once. Orbit 0.008 rad per
pixel; pan 0.0016 × orbit distance per

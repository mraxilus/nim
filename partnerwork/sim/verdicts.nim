...
func hugOn(state: State; link: Link; who: Body): float =
  ## Sum the turns a connection's rope hugs one body through, signed as the
  ## sim signs a turn: anticlockwise seen from above is positive.
  let laid = lay(state, link)
  if laid.isNone:
    return 0.0
  for piece in laid.get.pieces:
    if piece.kind == Kind.Hug and
        dist(piece.centre, state.stance[who].centre) < 1e-6:
      result += piece.turn / (2.0 * PI)
...
proc turnsJson(): string =
  ## Write every single and two-hand hold at every level through four half
  ## turns of the held body each way, as JSON for the drawn page to animate.
  ##   The page draws each state by its own grammar and reads the words --
  ##     what her arm has become, where the hands are, whether the rope
  ##     holds -- from here, so what it animates is the rope's answer and
  ##     not its own guess.  Counted from face to face.  `wrap` says which
  ##     sign of turn is the wrap way for that hold, read off the rope at
  ##     the first half turn, so the page need not know the sim's sign.
  func q(text: string): string = "\"" & text & "\""
  result = "{\"holds\":{"
  var first_hold = true
  for (one, two, name) in [
      (Arm.Left, Arm.Left, "L-l"), (Arm.Right, Arm.Right, "R-r"),
      (Arm.Left, Arm.Right, "L-r"), (Arm.Right, Arm.Left, "R-l")]:
    if not first_hold: result.add ","
    first_hold = false
    let probe = turned(settled(joined(one, two, LOW)), Body.Two, PI)
    let lying = lyingOn(probe, probe.links[0], Body.Two)
    let wrap = if lying.isSome and lying.get.aspect == Aspect.Fore: 1 else: -1
    result.add q(name) & ":{\"wrap\":" & $wrap & ","
    var first_level = true
    for (level, height) in LEVELS:
      if not first_level: result.add ","
      first_level = false
      result.add q(level) & ":["
      for halves in -4 .. 4:
        if halves > -4: result.add ","
        let
          base = settled(joined(one, two, height))
          spun = if halves == 0: base
                 else: turned(base, Body.Two, float(halves) * PI)
          her = said(lyingOn(spun, spun.links[0], Body.Two))
          him = said(lyingOn(spun, spun.links[0], Body.One))
        result.add "{\"t\":" & $(float(halves) / 2.0) & ",\"verdict\":" &
          q(short(spun)) & ",\"her\":" & q(her) & ",\"him\":" & q(him) &
          ",\"hands\":" & q(placed(spun, Body.Two)) &
          &",\"hug\":{hugOn(spun, spun.links[0], Body.Two):.3f}" & "}"
      result.add "]"
    result.add "}"
  result.add "},\"pairs\":{"
  var first_pair = true
  for (pair, name) in [
      (@[(Arm.Left, Arm.Left), (Arm.Right, Arm.Right)], "L-l.R-r"),
      (@[(Arm.Left, Arm.Right), (Arm.Right, Arm.Left)], "L-r.R-l")]:
    if not first_pair: result.add ","
    first_pair = false
    result.add q(name) & ":{"
    var first_level = true
    for (level, height) in LEVELS:
      ...
      for halves in -4 .. 4:
        if halves > -4: result.add ","
        let spun = if halves == 0: rested
                   else: turned(rested, Body.Two, float(halves) * PI)
        result.add "{\"t\":" & $(float(halves) / 2.0) & ",\"verdict\":" &
          q(short(spun)) & &",\"hugs\":[{hugOn(spun, spun.links[0], Body.Two):.3f}," &
          &"{hugOn(spun, spun.links[1], Body.Two):.3f}]" & "}"
      result.add "]"
    result.add "}"
  result.add "}}"


proc splice(page, json: string) =
  ## Put the turns JSON into the drawn page, where it keeps a slot for it.
  ##   The page is hand-drawn; only this one block of it is generated, so
  ##     the drawing and the rope's answers cannot drift apart unnoticed.
  const opening = "<script type=\"application/json\" id=\"turns\">"
  let text = readFile(page)
  let a = text.find(opening)
  if a < 0:
    echo "verdicts: " & page & " has no turns slot; not spliced"
    return
  let b = text.find("</script>", a)
  writeFile(page, text[0 ..< a + opening.len] & json & text[b .. ^1])
  echo "verdicts: turns spliced into " & page
...
when isMainModule:
  ...
  let turns = turnsJson()
  writeFile("design/turns.json", turns)
  splice("design/wholecloth.html", turns)

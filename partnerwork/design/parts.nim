...
func frameParts*(): Parts =
  ...
  # Drawn twice -- the orbit itself, where the walker keeps their side to
  # the centre (rule 32), and the compound, which is that orbit with a
  # counter-turn danced into it so the walker keeps their own bearing.
  ...
  # while turning the other way, so the walker keeps their own bearing --
  # lands in one picture whichever dancer walks it: only the pair's axis has
  # swung, and the drawing cannot say who did the walking.
  ...
  # And an orbit -- the walker keeping their side to the centre (rule 32) --
  # lands where their partner's own axis turn lands.  The compound does not,
...
const FAMILY_OF*: array[TurnWay, Family] = [
  ...
  ##   An orbit that faces the centre turns the walker as far as it carries
  ##     them (rule 32), so it comes to the same thing as their partner's
  ##     axis turn the other way: two rounds between the four ways, each
  ##     reached by one axis turn and one orbit.
  ##   Which is the point of the correction.  A half turn is then a half
  ##     turn however it is danced, and the ways can be equated.
...
func windSense*(way: TurnWay): float =
  ...
  ##   A way that winds nothing at all would take the forward sense, having
  ##     no end to walk off the side of.  Since rule 32 there is no such
  ##     way -- every one of the four winds -- and the fallback stands only
...
func handTurnParts*(): Parts =
  ...
  ##   Rules 28 and 31: seven positions, a half turn apart -- the frame, an
  ##     X either side of it, a diamond beyond each X, and a swan beyond
  ##     each diamond.
  ##   Rule 15: every position drawn, every edge animated, by every way of
  ##     turning.
  ##   Rule 32: all four of them walk the chain, since an orbit that keeps
...
const LINKS* = [
  (side: Side.Left, site: Site.LeftHand),
  (side: Side.Left, site: Site.RightHand),
  (side: Side.Right, site: Site.LeftHand),
  (side: Side.Right, site: Site.RightHand),
] ## Every connection there is, in the order the page reads them.


func ropeFigure(cls: string; side: Side; site: Site;
    winds = 0; extra: seq[string] = @[]): string =
  ## Draw one connection: the two bodies, the solved rope, the hands it joins.
  let
    laid = lie(HUMAN, GAP, side, site)
    path = drawn(laid)
  var bits = stance(HUMAN)
  if winds != 0:
    # The wind is drawn round the follow, who is the dancer a turn is led on.
    bits.add reachMarkup(@[spiral(HUMAN, at((0.0, GAP)), path[^1], winds)],
      INK[armOf(site)])
  bits.add ropeInk(path, side, site)
  bits.add handsOn(HUMAN, side, site)
  bits.add extra
  figureOf(cls, bits)


func pairFigure(cls: string; target: ontology.Frame;
    marked = false): string =
  ## Draw a frame that holds two connections, breaking the one that dives.
  ##   Which rope is cut is not a choice the drawing makes: `crossings` finds
  ##     where the two actually meet, and `Frame.over` says which of them is
  ##     on top when they do.
  var held: seq[tuple[side: Side, site: Site]] = @[]
  for side in Side:
    if target.hold[side].isSome:
      held.add (side, target.hold[side].get)
  doAssert held.len == 2, "A pair figure wants two connections."
  let
    a = drawn(lie(HUMAN, GAP, held[0].side, held[0].site))
    b = drawn(lie(HUMAN, GAP, held[1].side, held[1].site))
    meets = crossingsOf(a, b)
    a_over = target.over == some(held[0].side)
  # Only a rope that actually dives is broken.  `cutGap` cuts wherever it is
  # asked to, crossing or no crossing, so asking it unconditionally would put
  # a gap in a parallel pair that never meets -- and that gap would say the
  # one thing this figure exists to deny.
  doAssert (meets.len == 1) == target.over.isSome,
    &"A pair crosses where it has no over; got `{target.describe}`."
  var bits = stance(HUMAN)
  bits.add ropeInk(a, held[0].side, held[0].site,
    under = (if meets.len > 0 and not a_over: b else: @[]))
  bits.add ropeInk(b, held[1].side, held[1].site,
    under = (if meets.len > 0 and a_over: a else: @[]))
  for one in held:
    bits.add handsOn(HUMAN, one.side, one.site)
  if marked:
    for met in meets:
      bits.add &"""<circle cx="{n(met.x)}" cy="{n(met.y)}" r="4.5"""" &
        &""" fill="none" stroke="{QUIET}" stroke-width="1"/>"""
  figureOf(cls, bits)


func rigParts*(): Parts =
  ## Build every figure the rig page places.
  ##   Every one of them is the solver's own output put on the page: nothing
  ##     here is drawn by hand and then claimed to be what the model says.

  # All eight states, small, so the whole space is one glance.
  for target in FRAMES:
    var bits = stance(HUMAN)
    for side in Side:
      if target.hold[side].isSome:
        let site = target.hold[side].get
        bits.add ropeInk(drawn(lie(HUMAN, GAP, side, site)), side, site)
        bits.add handsOn(HUMAN, side, site)
    result[&"state_{target.slug}"] = figureOf("tiny", bits)

  # The four connections, each drawn the way the rope has to lie.
  for one in LINKS:
    result[&"lie_{leadName(one.side)}_{followName(one.site)}"] =
      ropeFigure("f", one.side, one.site)

  # Why `over` exists: the crossed pair meet, the parallel pair never do.
  result["pair_crossed"] = pairFigure("f", fromKey("lrL").get, marked = true)
  result["pair_parallel"] = pairFigure("f", fromKey("rl.").get, marked = true)
  doAssert result["pair_crossed"] != result["pair_parallel"],
    "The crossed pair and the parallel pair drew the same picture."

  # What a turn costs, and where the rope runs out.  The refused figure is
  # drawn refused: the coils are there and the rope that would pay for them
  # is not, so the reader sees the shortfall rather than a missing plate.
  let held = rope(Side.Left, Site.RightHand)
  for winds in 0 .. 2:
    let
      spent = windFollow(held, winds)
      over_by = -slack(HUMAN, GAP, spent)
      tag = if over_by <= 0.0: ""
            else: note((0.0, half() - 6.0),
              &"{metres(over_by)} m short", DEEP[Arm.R])
    result[&"wind_{winds}"] = ropeFigure("f", Side.Left, Site.RightHand,
      winds = winds, extra = (if tag.len > 0: @[tag] else: @[]))
  doAssert feasible(HUMAN, GAP, windFollow(held, 1)),
    "A half turn should fit at the smallest gap."
  doAssert not feasible(HUMAN, GAP, windFollow(held, 2)),
    "A whole turn should not fit at the smallest gap."

  # Over the head, the rope is on the axis and spends nothing, so shedding a
  # wrap gives back exactly what the wind was holding.  Drawn as the pair it
  # is -- wound, then let go -- because the law is the difference between two
  # pictures and not anything visible in either one alone.
  result["over_head_shed"] = ropeFigure("f", Side.Left, Site.RightHand,
    winds = 0)
  doAssert result["over_head_shed"] == result["wind_0"],
    "A shed wrap did not come back to where the rope started."
  doAssert liftable(HUMAN, GAP, held),
    "A lift should be afforded at the smallest gap."
  doAssert slack(HUMAN, GAP, shedFollow(windFollow(held, 1))) ==
    slack(HUMAN, GAP, held), "Shedding a wrap did not return its rope."
